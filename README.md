...
- Upload images, drag-and-drop, and pick from the local `input/` library.
...
- Output history in `output/history.json` (latest 500 items), download, favorite, and delete.
...
| `config/templates/` | User-created local templates |
| `config/templates-rh/` | User-saved RunningHub apps and RH templates |
| `input/`, `output/`, `uploads/`, `presets/` | Local user data |
...
- Upload ảnh, kéo-thả, chọn ảnh từ thư mục `input/`.
...
- Lưu lịch sử output trong `output/history.json` (500 mục gần nhất), tải về, favorite, xóa.
...
| `config/templates/` | Template người dùng tạo thêm (local) |
| `config/templates-rh/` | App RunningHub đã lưu và template RH |
| `input/`, `output/`, `uploads/`, `presets/` | Dữ liệu local |
...
Các thư mục `input/`, `output/`, `uploads/`, `presets/`, `config/templates/` và `config/templates-rh/` chứa dữ liệu local. Không commit dữ liệu riêng tư lên GitHub nếu không cần thiết.
